...
Class(Argon.Storage, 'Knex')({
      ...
      if (item instanceof Object) {
        ...
        for (property in item) {
          if (item.hasOwnProperty(property)) {
            sanitizedItem[_(property).camelize().value()] = item[property];
          }
        }
  ...
  function(data) {
    ...
    for (property in data) {
      if (data.hasOwnProperty(property)) {
        sanitizedData[_(property).underscored().value()] = data[property];
      }
    }
    ...
    init    : function init(config) {
      if (typeof config !== 'undefined') {
        ...
        if (typeof config.queries !== 'undefined') {
          Object.keys(config.queries || {}).forEcah(function(propertyName) {
            this[propertyName] = config.queries[propertyName];
          }, this.queries);
        }
      }
